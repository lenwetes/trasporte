...
import { ArrowLeft, AlertTriangle, ShieldAlert, Sparkles, AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/button";

export default async function NuevaSiniestroPage() {
        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-top-4 duration-1000 p-8 lg:p-12 bg-slate-50/10 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white border border-primary/10 p-10 shadow-2xl relative overflow-hidden group">
                ...
                <div className="flex items-center gap-6 relative z-10">
                    <div className="h-16 w-16 flex items-center justify-center border border-primary/10 bg-slate-50 text-red-600 shadow-inner group-hover:scale-110 transition-transform">
                        <AlertTriangle className="h-8 w-8 text-red-600 shadow-sm" />
                    </div>
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="h-1 w-6 bg-red-600/30" />
                            <span className="text-[11px] font-black uppercase tracking-[0.4em] text-red-600 tracking-tighter">Investigación de Accidentes</span>
                        </div>
                        ...
                        <p className="text-xs font-bold text-slate-900 mt-1 uppercase tracking-widest leading-relaxed">
                            Registro auditable oficial de siniestralidad <span className="text-red-500 underline font-mono">CODE:CRIT_EV</span>
                        </p>
                    </div>
                </div>

                <Link href="/dashboard/novedades?tab=siniestros">
            ...
            <div className="max-w-6xl mx-auto bg-white border border-primary/10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] p-10 lg:p-16">
                <div className="mb-12 border-b border-primary/5 pb-10 flex items-start gap-8">
                    <div className="flex-1 space-y-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-sm font-black text-primary uppercase tracking-widest">Protocolo de Recolección de Datos</h2>
                        </div>
                        <p className="text-[13px] font-medium text-slate-900 leading-relaxed max-w-2xl">
                            Este registro es el inicio del proceso de investigación de siniestralidad. Ingrese los detalles lo más precisamente posible
                            para facilitar la reconstrucción técnica del evento por parte del comité de seguridad vial.
                        </p>
                    </div>
                </div>
                ...
                <div className="relative z-10">
                    <SiniestroForm
                        ...
                        vehiculos={vehiculos}
                        defaultConductorId={userRole !== "ADMIN" && userRole !== "SECRETARIA" ? userId : undefined}
                    />
                </div>
            </div>
        </div>
